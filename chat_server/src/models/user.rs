...
impl AppState {
    ...
    /// Create a new user
    pub async fn user_create(&self, input: &CreateUser) -> Result<User, AppError> {
        ...
        let ws = match self.find_workspace_by_name(&input.workspace).await? {
            Some(ws) => ws,
            None => self.workspace_create(&input.workspace, 0).await?,
        };
    ...
    pub async fn user_verify(&self, input: &SigninUser) -> Result<Option<User>, AppError> {
        let user: Option<User> = sqlx::query_as(
...
mod tests {
    ...
    async fn create_duplicate_user_should_fail() -> Result<()> {
        ...
        let ret = state.user_create(&input).await;
        match ret {
    ...
    async fn create_and_verify_user_should_work() -> Result<()> {
        ...
        let user = state.user_create(&input).await?;
